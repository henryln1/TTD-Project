...
def create_single_txt_location(entry_line):
	...
	#complains when too little or too many things parsed out
...
def write_dict_to_new_file(file_name, dict_name):
	'''
	Takes a dictionary and writes it to text file where each line is a key/value pair
...
def check_validity(market_url_to_ads_txt_dict):

	'''
	Looks at each url and determines whether or not it leads to a valid ads.txt file
	'''
	for key in market_url_to_ads_txt_dict:
		url = market_url_to_ads_txt_dict[key]
		valid = check_valid_url_ad_txt(url)
		if valid:
			print(url + " is valid.")
		else:
...
def main(args):
	...
	check_validity(market_url_to_ads_txt_dict)
	return
	#TODO
...
if __name__ == "__main__":
	...
	file.py arg1 arg2 arg3

	arg0 is the python file itself that we're running
