...
from utils import parse_for_specific_parameter, check_missing_slash
...
class Extractor:
	...
	def look_for_ads_txt_url(self, entry_line):
		...
		def check_entry_site_ads_txt_url(site_entry):
			...
			if possible_url in self.url_cache:
				if self.url_cache[possible_url] is True:
					return possible_url
				else:
					return ''
			valid = check_valid_url_ad_txt(possible_url)
			self.url_cache[possible_url] = valid
		...
		entry_line = json.loads(entry_line.replace('\n\n', '\n'))
		possible_url = ''
